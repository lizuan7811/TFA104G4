...
-- -----------------------------------------------------
-- Table `Group4_db`.`like`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Group4_db`.`Like` (
  `likeID` INT AUTO_INCREMENT NOT NULL COMMENT '按讚ID',
  `diaryID` INT NOT NULL COMMENT '日誌ID',
  `custID` INT NOT NULL COMMENT '會員ID',
  `createdTime` TIMESTAMP NOT NULL COMMENT '創建時間',
  PRIMARY KEY (`likeID`),
...
CREATE TABLE IF NOT EXISTS `Group4_db`.`Temporder` (
  ...
  `ingrelID` INT NOT NULL COMMENT '食材編號 (ingreID): int, not null',
  `orderQuan` INT NOT NULL COMMENT '購買數量 (orderQuan): int, not null',
  `price` INT NOT NULL COMMENT '單價 (price): int, not null',
  PRIMARY KEY (`custID`, `ingrelID`),
  constraint FK_Temporder_Ingre_id FOREIGN KEY(`ingrelID`) REFERENCES Ingre(`idIngre`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Group4_db`.`OrderIngreList` (
  `idOrder` INT AUTO_INCREMENT NOT NULL COMMENT '訂單編號',
  `idIngre` INT AUTO_INCREMENT NOT NULL COMMENT '食材編號',
  `orderQuan` INT NOT NULL COMMENT '購買數量',
  `price` INT NOT NULL COMMENT '單價',
  PRIMARY KEY (`idOrder`, `idIngre`),
  constraint FK_OrderIngreList_Ingre_id FOREIGN KEY(`idIngre`) REFERENCES Ingre(`idIngre`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Group4_db`.`Address` (
  ...
  `createdTime` TIMESTAMP(6) NOT NULL COMMENT '創建時間',
  `default` TINYINT(1) NULL COMMENT '預設選項(0 : 非預設; 1 : 預設)',
...
CREATE TABLE IF NOT EXISTS `Group4_db`.`Card` (
  ...
  `createdTime` TIMESTAMP(6) NOT NULL COMMENT '創建時間',
  `default` TINYINT(1) NOT NULL COMMENT '預設選項(0 : 非預設; 1 : 預設)',
  PRIMARY KEY (`idCard`),
